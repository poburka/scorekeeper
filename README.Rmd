...
**raw_vars** : a raw variable or list of raw variables (comma separated, listed as a single character string) needed for an operation
...
**new_vals**: values to be recoded in a `recode` operation. Follow the convention 'old' = 'new', with commas separating each old/new pair. class(new_vals) should be a single charachter string
...
The required columns in a scoresheet currently have limited flexibility -- see documentation for the individual functions for details on these limitations. I anticipate adding additional functionality in the future. Enter `NA` for any elements that are unnecessary for a given operation.  
